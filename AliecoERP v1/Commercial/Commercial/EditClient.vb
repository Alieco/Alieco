...
Public Class EditClient
    ...
    Private Sub ButtonX1_Click(sender As System.Object, e As System.EventArgs) Handles ButtonX1.Click
        Try
            Dim q As String = "update commercial_client set nconsult= '" & nconslt.Text & "',codeCl = '" & code.Text & "',Date = '" & datenais.Value & "' ,NomClient = '" & mysql_escape_string(nomcl.Text) & "' ,Fonction='" & mysql_escape_string(fonction.Text) & "' ,RaisonSocial= '" & mysql_escape_string(rs.Text) & "' ,Adresse= '" & mysql_escape_string(adresse.Text) & "' ,NumTel= '" & numtel.Text & "' , Fax='" & fax.Text & " ' ,Email= '" & mysql_escape_string(email.Text) & "',NIdentif_fiscale= '" & NIF.Text & "', NArticle_imposition= '" & NAI.Text & "',Description_Consult = '" & mysql_escape_string(description.Text) & "', Observation='" & mysql_escape_string(obs.Text) & "', responsable='" & mysql_escape_string(responsable.Text) & "', contrainte='" & mysql_escape_string(contrainte.Text) & "', faisabilite='" & mysql_escape_string(faisabilite.Text) & "', delai_reponse='" & delai.Text & "',filière='" & mysql_escape_string(activite.Text) & "',secteur='" & mysql_escape_string(secteur.Text) & "'where idclient='" & idclient & "'"
            ' Clipboard.SetText(q)
