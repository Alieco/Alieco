    ...
    Private Sub ButtonX1_Click(sender As System.Object, e As System.EventArgs) Handles ButtonX1.Click
        Try
            ...
            Try
                ...
                IsConnected("SELECT distinct(employes.Matricule),nom,prénom,Fonction,Direction,Hall,total_gain,moi_annee FROM employes,paie_mensuel WHERE  Employes.Matricule=paie_mensuel.Matricule and paie_mensuel.type='P'  and moi_annee='" & Moi.Text & Anne.Text & "'and fonction LIKE '%" & fonction.Text & "%' and direction LIKE '%" & direction.Text & "%' order by Matricule asc ", False)
                Clipboard.SetText("SELECT distinct(employes.Matricule),nom,prénom,Fonction,Direction,Hall,total_gain,moi_annee FROM employes,paie_mensuel WHERE  Employes.Matricule=paie_mensuel.Matricule and paie_mensuel.type='P'  and moi_annee='" & Moi.Text & Anne.Text & "'and fonction LIKE '%" & fonction.Text & "%' and direction LIKE '%" & direction.Text & "%' order by Matricule asc ")
                If myDR.HasRows Then
                    While myDR.Read

                        ds1.EtatPaiementDT.AddEtatPaiementDTRow(IIf(IsDBNull(myDR("Matricule")), "", myDR("Matricule")), IIf(IsDBNull(myDR("Nom") & " " & myDR("Prénom")), "", myDR("Nom") & " " & myDR("Prénom")), IIf(IsDBNull(myDR("Fonction")), "", myDR("Fonction")), IIf(IsDBNull(myDR("Direction")), "", myDR("Direction")), IIf(IsDBNull(myDR("Hall")), "", myDR("Hall")), IIf(IsDBNull(myDR("total_gain")), "", myDR("total_gain")), IIf(IsDBNull(myDR("moi_annee")), "", myDR("moi_annee")))
                    End While
                End If
            Catch ex As Exception
                EnvoiError(ex.Message)
